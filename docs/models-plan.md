...
| Model objects | 2.6 | ❌ `WESceneObject` has no `model` key; the objects are dropped silently | 254 objects, 16 scenes |
...
| Camera layers | 2.3 | ❌ not decoded; the camera never moves | 15 layers, 7 scenes |
| Scene camera paths, `camerafade` | 2.2 | ❌ | 7 scenes (6 default projects, 2350874185) |
...
**M0 — Format and seams (one agent, first, small; no output change).**
...
**M1 — The `.mdl` parser (one agent; parallel with M0).**
- **Files:** new `Scene/Format/MDLDocument.swift` (the value types: meshes, vertex format with the attribute table of §1.2, skeleton, clips, attachments, morph targets, reference pose), new `Scene/Format/MDLReader.swift` (every version and section of §1, strict: an overrun or a fast-fail condition is a logged decode error for that model, never a crash), new `Scene/Format/MDLVertexFormat.swift`.
- **Tests:**
  - `MDLParseTests`: small fixture files in `Tests/Fixtures/Models/` (one of each tag set; the smallest library files are 1–3 KB, or hand-built ones) against values from `mdl.py`; truncated and oversized inputs; 129 bones rejected.
  - `MDLLibrarySweepTests` (`OWE_LIBRARY`): every library `.mdl` parses, and per file the counts (meshes, formats, strides, vertices, indices, bones, clips, frames, fps, modes, attachments) equal `sweep.json`, checked in as `Tests/Fixtures/Models/mdl-sweep.json`.
  - A decode equality check on 3 files (a static mesh, a skinned 3D model, a puppet): positions, blend indices and weights, and the first clip's frame-0 TRS against `mdl.py --decode` dumps.
