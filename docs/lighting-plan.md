...
- **Not done:** an HDR display output (an `rgba16Float` `CAMetalLayer` with extended linear sRGB, RV from the screen's EDR headroom), which needs an EDR display to verify; scripts driving `bloomhdr*` (2350874185 binds a property script to `bloomhdrstrength`; `thisScene` has no such member here, so its authored value is used); `ccsimple` and the camera fade (steps 6–7), as in B1; A4's prelit image (`ImageMaterialRenderer.prelitFormat`) is still RGBA8 in HDR, where WE's layer buffers are RGBA16F.
