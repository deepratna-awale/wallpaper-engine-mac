...
| `#require LightingV1` | generated per `LIGHTS_*` combo set (2.1) | ❌ `ShaderSource.stubRequires` (`Shaders/ShaderSource.swift`:211) emits `return color * ambient`, where "ambient" is really f0 | every LIGHTING material |
| `LIGHTS_*`, `HDR`, `SCENE_ORTHO` engine combos | set per material (2.2) | ❌ | — |
