...
- **Not done:** an HDR display output (an `rgba16Float` `CAMetalLayer` with extended linear sRGB, RV from the screen's EDR headroom), which needs an EDR display to verify; `ccsimple` and the camera fade (steps 6–7): WE makes `ccsimple` only for colour-correction settings away from identity (0x1401826a2…0x1401826f3) and the fade only for scenes with camera paths (0x140181bae), so at WE's defaults and in this library neither draws. Done since: scripts and timelines drive `bloomhdr*` (bound-only scene fields, `69f9530`), and the prelit image is RGBA16F in HDR (`961c6f4`).
