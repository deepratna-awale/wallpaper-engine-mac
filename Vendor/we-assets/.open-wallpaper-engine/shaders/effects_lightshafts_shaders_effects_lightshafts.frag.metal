...
fragment main0_out main0(main0_in in [[stage_in]], constant float2& g_Scale [[buffer(0)]], constant float2& g_Feather [[buffer(1)]], constant float& g_Time [[buffer(2)]], constant float& g_Speed [[buffer(3)]], constant float& g_Exponent [[buffer(4)]], constant float& g_Smoothness [[buffer(5)]], constant float3& g_ColorRaysStart [[buffer(6)]], constant float3& g_ColorRaysEnd [[buffer(7)]], constant float& g_Intensity [[buffer(8)]], texture2d<float> g_Texture0 [[texture(0)]], texture2d<float> g_Texture1 [[texture(1)]], sampler g_Texture0Smplr [[sampler(0)]], sampler g_Texture1Smplr [[sampler(1)]])
{
    main0_out out = {};
    ...
    float3 fxColor = mix(g_ColorRaysStart, g_ColorRaysEnd, float3(fxCoordRef.y));
    fx *= mask;
    int param = 31;
    float3 param_1 = albedo.xyz;
    float3 param_2 = fxColor * g_Intensity;
    float param_3 = fx;
    float3 _199 = ApplyBlending(param, param_1, param_2, param_3);
    albedo.x = _199.x;
    albedo.y = _199.y;
    albedo.z = _199.z;
