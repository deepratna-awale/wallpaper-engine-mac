...
fragment main0_out main0(main0_in in [[stage_in]], constant float& g_Time [[buffer(0)]], constant float& g_DistortionStrength [[buffer(1)]], constant float& g_DistortionWidth [[buffer(2)]], constant float& g_DistortionSpeed [[buffer(3)]], constant float& g_NoiseAlpha [[buffer(4)]], constant float& g_ArtifactsScale [[buffer(5)]], constant float& g_NoiseScale [[buffer(6)]], constant float& g_Chromatic [[buffer(7)]], constant float& g_Tracking [[buffer(8)]], texture2d<float> g_Texture1 [[texture(0)]], texture2d<float> g_Texture0 [[texture(1)]], sampler g_Texture1Smplr [[sampler(0)]], sampler g_Texture0Smplr [[sampler(1)]])
{
    main0_out out = {};
    ...
    float artifactLimiter = powr(fast::max(g_ArtifactsScale, 9.9999997473787516355514526367188e-05), 0.20000000298023223876953125);
    float artifactsAlpha = (((step(0.001000000047497451305389404296875, g_NoiseScale) * step(0.89999997615814208984375, vhsNoise.x * artifactLimiter)) * step(0.89999997615814208984375, vhsNoise2.x * artifactLimiter)) * vhsNoise.y) * vhsNoise2.y;
    float artifactLimiterChromatic = powr(fast::max(g_Chromatic, 9.9999997473787516355514526367188e-05), 0.20000000298023223876953125);
    ...
    float xOffset = ((g_NoiseAlpha * artifactsAlphaChromatic) * g_Chromatic) * 0.100000001490116119384765625;
    float lineNoise = g_Texture1.sample(g_Texture1Smplr, float2(g_Time, in.v_TexCoordVHSNoise.w)).x;
    float lineOffset = (step(0.89999997615814208984375, lineNoise) * 0.004999999888241291046142578125) * g_Tracking;
    xOffset += lineOffset;
    float4 _315 = glitchCoords;
    float2 _317 = _315.xz + float2(lineOffset);
    glitchCoords.x = _317.x;
    glitchCoords.z = _317.y;
    texCoord.x += xOffset;
    ...
    float param_2 = blend;
    float3 _385 = ApplyBlending(12, param, param_1, param_2);
    albedo.x = _385.x;
    albedo.y = _385.y;
    albedo.z = _385.z;
    float4 _392 = albedo;
    float4 _394 = albedo;
    float3 _406 = mix(_392.xyz, fast::min(_394.xyz + smoothstep(float3(0.699999988079071044921875), float3(1.0), _noise), float3(1.0)), float3(blend));
    albedo.x = _406.x;
    albedo.y = _406.y;
    albedo.z = _406.z;
    float4 _413 = albedo;
    float4 _415 = albedo;
    float3 _421 = mix(_413.xyz, float3(1.0) - _415.xyz, float3(artifactsAlpha));
    albedo.x = _421.x;
    albedo.y = _421.y;
    albedo.z = _421.z;
    out.out_FragColor = mix(orig, albedo, float4(g_NoiseAlpha * vhsBlend));
    return out;
}
