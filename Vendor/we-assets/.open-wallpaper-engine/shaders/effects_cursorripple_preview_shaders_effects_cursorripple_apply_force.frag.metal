...
fragment main0_out main0(main0_in in [[stage_in]], constant float& g_Frametime [[buffer(0)]], constant float4& g_PointerState [[buffer(1)]], texture2d<float> g_Texture0 [[texture(0)]], sampler g_Texture0Smplr [[sampler(0)]])
{
    ...
    float2 unprojectedUVsLast = in.v_PointerUVLast.xy / float2(in.v_PointerUVLast.z);
    unprojectedUVs += float2(0.5);
    unprojectedUVs.y = 1.0 - unprojectedUVs.y;
    unprojectedUVsLast += float2(0.5);
    ...
    distLDelta = fast::max(9.9999997473787516355514526367188e-05, distLDelta);
    ...
    float timeAmt = g_Frametime / 0.0199999995529651641845703125;
