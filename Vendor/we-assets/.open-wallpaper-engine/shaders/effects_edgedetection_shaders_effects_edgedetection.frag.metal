...
fragment main0_out main0(main0_in in [[stage_in]], constant float3& g_OutlineColor2 [[buffer(0)]], constant float3& g_OutlineColor1 [[buffer(1)]], constant float& g_DetectionThreshold [[buffer(2)]], constant float& g_DetectionMultiply [[buffer(3)]], constant float& g_BlendBrightness [[buffer(4)]], constant float& g_BlendAlpha [[buffer(5)]], texture2d<float> g_Texture0 [[texture(0)]], sampler g_Texture0Smplr [[sampler(0)]])
{
    main0_out out = {};
    ...
    float3 combinedColor = mix(g_OutlineColor2, g_OutlineColor1, float3(fast::min(1.0, fast::max(0.0, g - g_DetectionThreshold) * g_DetectionMultiply))) * g_BlendBrightness;
    out.out_FragColor.w = albedo.w;
    float3 param_2 = albedo.xyz;
    float3 param_3 = combinedColor;
    float param_4 = g_BlendAlpha;
    float3 _187 = ApplyBlending(0, param_2, param_3, param_4);
    out.out_FragColor.x = _187.x;
    out.out_FragColor.y = _187.y;
    out.out_FragColor.z = _187.z;
    return out;
}
