...
static inline __attribute__((always_inline))
void ClipSpaceToScreenSpace(thread const float4& clipSpacePosition, thread float3& screenSpacePosition)
{
...
vertex main0_out main0(main0_in in [[stage_in]], constant float4x4& g_ModelMatrix [[buffer(0)]], constant float3x3& g_NormalModelMatrix [[buffer(1)]], constant float3& g_LightSkylightColor [[buffer(2)]], constant float3& g_LightAmbientColor [[buffer(3)]], constant float4x4& g_ViewProjectionMatrix [[buffer(4)]], constant float3& g_EyePosition [[buffer(5)]])
{
    main0_out out = {};
    ...
    float3 _133 = g_EyePosition - worldPos.xyz;
    out.v_ViewDir.x = _133.x;
    out.v_ViewDir.y = _133.y;
    out.v_ViewDir.z = _133.z;
    out.v_ViewDir.w = worldPos.y;
    out.v_WorldNormal = worldNormal;
    float4 param_6 = out.gl_Position;
    float3 param_7;
    ClipSpaceToScreenSpace(param_6, param_7);
    out.v_ScreenPos = param_7;
    float3 param_8 = worldNormal;
    out.v_LightAmbientColor = ApplyAmbientLighting(param_8, g_LightSkylightColor, g_LightAmbientColor);
    return out;
}
