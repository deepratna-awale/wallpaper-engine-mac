...
static inline __attribute__((always_inline))
float4 blurRadial13a(thread const float2& u, thread float& amt, constant float2& u_Center, texture2d<float> g_Texture0, sampler g_Texture0Smplr)
{
    float2 center = u_Center;
    ...
    float2 param = delta;
    float param_1 = o1;
    float2 r1 = rotateVec2(param, param_1) - delta;
    float2 param_2 = delta;
    float param_3 = o2;
    float2 r2 = rotateVec2(param_2, param_3) - delta;
    float2 param_4 = delta;
    float param_5 = o3;
    float2 r3 = rotateVec2(param_4, param_5) - delta;
...
fragment main0_out main0(main0_in in [[stage_in]], constant float2& u_Center [[buffer(0)]], constant float& u_Scale [[buffer(1)]], texture2d<float> g_Texture0 [[texture(0)]], sampler g_Texture0Smplr [[sampler(0)]])
{
    ...
    float param_1 = u_Scale;
    float4 _180 = blurRadial13a(param, param_1, u_Center, g_Texture0, g_Texture0Smplr);
    float4 albedo = _180;
