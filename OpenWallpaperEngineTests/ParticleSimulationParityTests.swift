...
final class ParticleSimulationParityTests: XCTestCase {
    ...
    func testSwiftAndMetalLayoutsAgree() throws {
        ...
        let sizes = try XCTUnwrap(device.makeBuffer(length: 7 * 4, options: .storageModeShared))
        ...
        let metal = Array(UnsafeBufferPointer(start: sizes.contents().bindMemory(to: UInt32.self, capacity: 7), count: 7)).map(Int.init)
        XCTAssertEqual(metal, [MemoryLayout<ParticleGPUState>.stride, MemoryLayout<ParticleGPUParameters>.stride,
                               ...
                               MemoryLayout<ParticleGPUInstance>.stride])
    }
