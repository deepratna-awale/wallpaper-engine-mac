...
/// particle shaders and its pipelines built, with and without a sprite sheet. Skipped when the
/// library is absent (CI). `OWE_LIBRARY` overrides the library root.
final class ParticleMaterialSweepTests: XCTestCase {
    func testEveryLibraryParticleMaterialBuilds() throws {
        ...
        for id in try FileManager.default.contentsOfDirectory(atPath: library.path).sorted() {
            ...
            let builder = ParticleMaterialPlanBuilder(translator: translator, readFile: read, loadTexture: { _, _ in nil })
            for object in scene.objects {
                ...
                for spriteSheet in [nil, sheet] {
                    ...
                    do {
                        ...
                        for stage in plan.stages where !stage.readsSceneSnapshot {
                            pipelines += 1
        ...
        print("Particle material sweep: \(systems) systems, \(pipelines) pipelines, \(failures.count) failures")
