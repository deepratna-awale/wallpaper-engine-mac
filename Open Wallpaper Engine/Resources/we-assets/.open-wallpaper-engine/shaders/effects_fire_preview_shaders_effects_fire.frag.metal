...
fragment main0_out main0(main0_in in [[stage_in]], constant float& g_Time [[buffer(0)]], constant float& g_FlowSpeed [[buffer(1)]], constant float& g_CloudScale [[buffer(2)]], constant float& g_CloudLOD [[buffer(3)]], constant float3& g_Color2 [[buffer(4)]], constant float3& g_Color1 [[buffer(5)]], constant float& g_CloudThreshold [[buffer(6)]], constant float& g_CloudFeather [[buffer(7)]], constant float& g_CloudsAlpha [[buffer(8)]], texture2d<float> g_Texture1 [[texture(0)]], texture2d<float> g_Texture2 [[texture(1)]], texture2d<float> g_Texture0 [[texture(2)]], sampler g_Texture1Smplr [[sampler(0)]], sampler g_Texture2Smplr [[sampler(1)]], sampler g_Texture0Smplr [[sampler(2)]])
{
    ...
    baseUV += ((((mix(flowMask, -flowMask, float2(streamNoise)) * cloudBackground) * 0.5) * streamNoise) * flowMaskLength);
    ...
    float blendNoise = smoothstep(0.0, 0.5, streamNoise) * smoothstep(1.0, 0.5, streamNoise);
    float3 cloudColor = mix(g_Color2, g_Color1, float3(blendNoise));
    ...
    float3 _211 = ApplyBlending(param, param_1, param_2, param_3);
    albedo.x = _211.x;
    albedo.y = _211.y;
    albedo.z = _211.z;
