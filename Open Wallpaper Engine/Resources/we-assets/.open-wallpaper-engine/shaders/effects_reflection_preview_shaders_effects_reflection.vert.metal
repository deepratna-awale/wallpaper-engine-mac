#pragma clang diagnostic ignored "-Wmissing-prototypes"

#include <metal_stdlib>
#include <simd/simd.h>

using namespace metal;

struct main0_out
{
    float4 v_TexCoord [[user(locn0)]];
    float2 v_ReflectedCoord [[user(locn1)]];
    float4 gl_Position [[position]];
};

struct main0_in
{
    float3 a_Position [[attribute(0)]];
    float2 a_TexCoord [[attribute(1)]];
};

static inline __attribute__((always_inline))
float4 mul(thread const float4& value, thread const float4x4& matrix)
{
    return matrix * value;
}

static inline __attribute__((always_inline))
float2 rotateVec2(thread const float2& value, thread const float& angle)
{
    float s = sin(angle);
    float c = cos(angle);
    return float2((value.x * c) - (value.y * s), (value.x * s) + (value.y * c));
}

vertex main0_out main0(main0_in in [[stage_in]], constant float4x4& g_ModelViewProjectionMatrix [[buffer(0)]], constant float4& g_Texture1Resolution [[buffer(1)]], constant float& g_ReflectionOffset [[buffer(2)]], constant float& g_Direction [[buffer(3)]])
{
    main0_out out = {};
    float4 param = float4(in.a_Position, 1.0);
    float4x4 param_1 = g_ModelViewProjectionMatrix;
    out.gl_Position = mul(param, param_1);
    out.v_TexCoord.x = in.a_TexCoord.x;
    out.v_TexCoord.y = in.a_TexCoord.y;
    float _92 = out.v_TexCoord.x;
    float _104 = out.v_TexCoord.y;
    float2 _112 = float2((_92 * g_Texture1Resolution.z) / g_Texture1Resolution.x, (_104 * g_Texture1Resolution.w) / g_Texture1Resolution.y);
    out.v_TexCoord.z = _112.x;
    out.v_TexCoord.w = _112.y;
    float2 center = float2(0.5);
    float2 delta = in.a_TexCoord - center;
    delta.y += g_ReflectionOffset;
    delta.y = -delta.y;
    float2 param_2 = delta;
    float param_3 = g_Direction;
    delta = rotateVec2(param_2, param_3);
    out.v_ReflectedCoord = center + delta;
    return out;
}

