...
fragment main0_out main0(main0_in in [[stage_in]], texture2d<float> g_Texture0 [[texture(0)]], sampler g_Texture0Smplr [[sampler(0)]])
{
    ...
    for (int i = 0; i < 4; i++)
    {
        float4 sampleValue = g_Texture0.sample(g_Texture0Smplr, v_TexCoord[i]);
        result += (sampleValue * sampleValue.w);
        weight += sampleValue.w;
    }
    float3 _67 = result.xyz / float3(fast::max(0.001000000047497451305389404296875, weight));
    out.out_FragColor.x = _67.x;
    out.out_FragColor.y = _67.y;
    out.out_FragColor.z = _67.z;
    out.out_FragColor.w = result.w / 4.0;
    return out;
}
