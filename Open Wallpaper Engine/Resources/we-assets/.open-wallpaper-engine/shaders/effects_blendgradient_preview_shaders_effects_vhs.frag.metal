...
struct main0_in
{
    ...
    float4 v_TexCoordGlitch [[user(locn2)]];
    float4 v_TexCoordNoise [[user(locn3)]];
    float4 v_TexCoordVHSNoise [[user(locn4)]];
};
...
fragment main0_out main0(main0_in in [[stage_in]], constant float& g_Time [[buffer(0)]], constant float& g_DistortionStrength [[buffer(1)]], constant float& g_DistortionWidth [[buffer(2)]], constant float& g_DistortionSpeed [[buffer(3)]], constant float& g_NoiseAlpha [[buffer(4)]], constant float& g_ArtifactsScale [[buffer(5)]], constant float& g_NoiseScale [[buffer(6)]], texture2d<float> g_Texture0 [[texture(0)]], texture2d<float> g_Texture1 [[texture(1)]], sampler g_Texture0Smplr [[sampler(0)]], sampler g_Texture1Smplr [[sampler(1)]])
{
    main0_out out = {};
    ...
    float param_2 = blend;
    float3 _278 = ApplyBlending(12, param, param_1, param_2);
    albedo.x = _278.x;
    albedo.y = _278.y;
    albedo.z = _278.z;
    float4 _285 = albedo;
    float4 _287 = albedo;
    float3 _299 = mix(_285.xyz, fast::min(_287.xyz + smoothstep(float3(0.699999988079071044921875), float3(1.0), _noise), float3(1.0)), float3(blend));
    albedo.x = _299.x;
    albedo.y = _299.y;
    albedo.z = _299.z;
    ...
    float4 _350 = albedo;
    float4 _352 = albedo;
    float3 _358 = mix(_350.xyz, float3(1.0) - _352.xyz, float3(artifactsAlpha));
    albedo.x = _358.x;
    albedo.y = _358.y;
    albedo.z = _358.z;
