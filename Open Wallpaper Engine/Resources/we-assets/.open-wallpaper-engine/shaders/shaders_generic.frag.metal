...
fragment main0_out main0(main0_in in [[stage_in]], constant float& g_Roughness [[buffer(0)]], constant float& g_Metallic [[buffer(1)]], constant spvUnsafeArray<float4, 4>& g_LightsColorRadius [[buffer(2)]], constant float& g_Light [[buffer(6)]], texture2d<float> g_Texture0 [[texture(0)]], sampler g_Texture0Smplr [[sampler(0)]])
{
    main0_out out = {};
    ...
    float3 _176 = ComputeLightSpecular(normal, in.v_Light0DirectionL3X.xyz, g_LightsColorRadius[0].xyz, g_LightsColorRadius[0].w, viewDir, specularPower, specularStrength, g_Light, g_Metallic, param);
    specularResult = param;
    float3 light = _176;
    float3 param_1 = specularResult;
    float3 _195 = ComputeLightSpecular(normal, in.v_Light1DirectionL3Y.xyz, g_LightsColorRadius[1].xyz, g_LightsColorRadius[1].w, viewDir, specularPower, specularStrength, g_Light, g_Metallic, param_1);
    specularResult = param_1;
    light += _195;
    float3 param_2 = specularResult;
    float3 _216 = ComputeLightSpecular(normal, in.v_Light2DirectionL3Z.xyz, g_LightsColorRadius[2].xyz, g_LightsColorRadius[2].w, viewDir, specularPower, specularStrength, g_Light, g_Metallic, param_2);
    specularResult = param_2;
    light += _216;
    float3 param_3 = specularResult;
    float3 _242 = ComputeLightSpecular(normal, float3(in.v_Light0DirectionL3X.w, in.v_Light1DirectionL3Y.w, in.v_Light2DirectionL3Z.w), g_LightsColorRadius[3].xyz, g_LightsColorRadius[3].w, viewDir, specularPower, specularStrength, g_Light, g_Metallic, param_3);
    specularResult = param_3;
    light += _242;
    light += in.v_LightAmbientColor;
    float4 _250 = albedo;
    float3 _255 = (_250.xyz * light) + specularResult;
    albedo.x = _255.x;
    albedo.y = _255.y;
    albedo.z = _255.z;
