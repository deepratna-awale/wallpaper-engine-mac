...
fragment main0_out main0(main0_in in [[stage_in]], constant float& g_CloudThreshold [[buffer(0)]], constant float& g_CloudFeather [[buffer(1)]], constant float& g_CloudsAlpha [[buffer(2)]], texture2d<float> g_Texture0 [[texture(0)]], texture2d<float> g_Texture1 [[texture(1)]], sampler g_Texture0Smplr [[sampler(0)]], sampler g_Texture1Smplr [[sampler(1)]])
{
    main0_out out = {};
    float4 albedo = g_Texture0.sample(g_Texture0Smplr, in.v_TexCoord.xy);
    float cloud0 = g_Texture1.sample(g_Texture1Smplr, in.v_TexCoordClouds.xy).x;
    float cloud1 = g_Texture1.sample(g_Texture1Smplr, in.v_TexCoordClouds.zw).x;
    ...
    float param_3 = blend;
    float3 _91 = ApplyBlending(param, param_1, param_2, param_3);
    albedo.x = _91.x;
    albedo.y = _91.y;
    albedo.z = _91.z;
    albedo.w = 1.0;
