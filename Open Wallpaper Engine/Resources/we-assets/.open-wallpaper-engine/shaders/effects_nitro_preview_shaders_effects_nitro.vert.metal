...
vertex main0_out main0(main0_in in [[stage_in]], constant float4x4& g_ModelViewProjectionMatrix [[buffer(0)]], constant float4& g_Texture0Resolution [[buffer(1)]], constant float& g_Time [[buffer(2)]], constant float4& g_NitroSpeeds [[buffer(3)]], constant float2& g_NitroScales [[buffer(4)]])
{
    main0_out out = {};
    ...
    float2 _64 = (in.a_TexCoord + (g_NitroSpeeds.xy * g_Time)) * g_NitroScales.x;
    out.v_TexCoordNitro.x = _64.x;
    out.v_TexCoordNitro.y = _64.y;
    float2 _78 = (in.a_TexCoord + (g_NitroSpeeds.zw * g_Time)) * g_NitroScales.y;
