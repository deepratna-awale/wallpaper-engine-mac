...
vertex main0_out main0(main0_in in [[stage_in]], constant float4x4& g_ModelMatrix [[buffer(0)]], constant float4x4& g_ViewProjectionMatrix [[buffer(1)]], constant float3x3& g_NormalModelMatrix [[buffer(2)]], constant float3& g_EyePosition [[buffer(3)]], constant float3& g_LightSkylightColor [[buffer(4)]], constant float3& g_LightAmbientColor [[buffer(5)]])
{
    main0_out out = {};
    ...
    float3x3 param_5 = g_NormalModelMatrix;
    float3 normal = fast::normalize(mul(param_4, param_5));
    out.v_TexCoord = in.a_TexCoord;
    float3 _92 = g_EyePosition - worldPos.xyz;
    out.v_ViewDir.x = _92.x;
    out.v_ViewDir.y = _92.y;
    out.v_ViewDir.z = _92.z;
