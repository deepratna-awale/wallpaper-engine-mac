...
struct main0_out
{
    ...
    float2 v_TexCoordGlitchBase [[user(locn1)]];
...
vertex main0_out main0(main0_in in [[stage_in]], constant float4x4& g_ModelViewProjectionMatrix [[buffer(0)]], constant float4& g_Texture0Resolution [[buffer(1)]], constant float& g_Time [[buffer(2)]], constant float& g_NoiseScale [[buffer(3)]], constant float& g_ArtifactsScale [[buffer(4)]], constant float& g_Chromatic [[buffer(5)]], constant float& g_NoiseAlpha [[buffer(6)]])
{
    ...
    float3 glitchOffset = (smoothstep(float3(0.0), float3(2.0), float3(1.0) + (sin((float3(11.0, 7.0, 13.0) * g_Time) * 2.0) * 0.5)) * g_Chromatic) * float3(0.0019000000320374965667724609375, 0.00209999992512166500091552734375, 0.001700000022538006305694580078125);
    ...
    out.v_TexCoordGlitchBase.x = out.v_TexCoord.x + (glitchOffset.z * fast::min(1.0, g_NoiseAlpha));
    out.v_TexCoordGlitchBase.y = out.v_TexCoord.y - (glitchOffset.z * fast::min(1.0, g_NoiseAlpha));
    return out;
}
